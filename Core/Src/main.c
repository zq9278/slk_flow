...
int main(void)
{
  ...
  LCD1602_Init();
    L6219_Init();
    /*
     include_directories(Hardware/HX711 Hardware/LCD1602 Hardware/MOTO Core/Inc Drivers/STM32F4xx_HAL_Driver/Inc Drivers/STM32F4xx_HAL_Driver/Inc/Legacy Drivers/CMSIS/Device/ST/STM32F4xx/Include Drivers/CMSIS/Include)

add_definitions(-DDEBUG -DUSE_HAL_DRIVER -DSTM32F429xx)

file(GLOB_RECURSE SOURCES "Core/*.*" "Drivers/*.*" "Hardware/*.*")
     */
    HAL_UART_Transmit(&huart4,"kaishi",strlen("kaishi"),100);
  /* USER CODE END 2 */
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_3|GPIO_PIN_5
  ...
  /*Configure GPIO pins : PB11 PB12 PB8 */
...
void delay_us(uint32_t micros) {
    __HAL_TIM_SET_COUNTER(&htim4, 0); // ?????????????? 0
    HAL_TIM_Base_Start(&htim4);
    while (__HAL_TIM_GET_COUNTER(&htim4) < micros); // ??????????? micros
